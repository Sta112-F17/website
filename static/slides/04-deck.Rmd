...
email <- email %>%
  mutate(
    spam_ind = ifelse(spam == 0, "not spam", "spam"),
    re_subj_ind = ifelse(re_subj == 0, "no", "yes")
  )
```


```{r echo=FALSE, fig.width = 8}
p1 <- ggplot(email, aes(x = spam_ind, fill = factor(re_subj_ind))) +
  geom_bar(position = "fill") +
  labs(x = "", fill = "re:", title = 'position: "fill"')
p2 <- ggplot(email, aes(x = spam_ind, fill = factor(re_subj_ind))) +
...
<div class="question">
**TEAM:** Create a list of the variables we would need in order to produce the following map depicting where Hurricane Irma is headed. (See original at https://nyti.ms/2x7nKLD.)
</div>
